...
def extract_attribute_token(data):
    token_counter = Counter()
    for img in data:
        for obj in img['attributes']:
            if "attributes" in obj:
                for attribute in obj['attributes']:
                    #polished_attr = sentence_preprocess(attribute)
...
def process_attribute (attribute_list):
  ...
  for attribute in attribute_list:
    ...
    # excluded tags
    excluded_tags = {"ADV", "DET", "SYM", "VERB"}

    new_attr = []
    for token in nlp(attribute):
        if token.pos_ not in excluded_tags:
            new_attr.append(token.text)
    attribute = " ".join(new_attr)
    return_list.append(attribute)
  return return_list

def preprocess_attribute(data):

    for img in tqdm(data):
        for obj in img['attributes']:
            if "attributes" in obj:
                attributes = process_attribute(obj['attributes'])
#                attributes = []
#                for attribute in obj['attributes']:
#                    polished_attr = process_attribute(attribute)
#                    attributes.append(polished_attr)
...
def encode_attributes(attr_data, token_to_idx, obj_data):
    """MUST BE CALLED AFTER encode_objects!!!"""
    #encoded_attibute = []  # encoded attributes 
    encoded_obj_attr = []  # encoded relationship tuple
    im_to_first_attr = np.zeros(len(attr_data), dtype=np.int32)
    im_to_last_attr = np.zeros(len(attr_data), dtype=np.int32)
    attr_idx_counter = 0
    ...
    #predicate_filtered = 0
    duplicate_filtered = 0
    for i, img in enumerate(attr_data):
        im_to_first_attr[i] = attr_idx_counter
        id_to_idx = obj_data[i]['id_to_idx']  # object id to object list idx
        for obj in img['attributes']:
            subj = obj['object_id']
            no_attr = False
            if "attributes" in obj:
                attr_list = obj['attributes']
            else:
                no_attr = True
            if obj['object_id'] not in id_to_idx:
                obj_filtered += 1
                continue
            elif no_attr:
                #l'oggetto non ha attributi
                continue
            else:
                for attribute in attr_list:
                    #polished_attr = sentence_preprocess(attribute)
                    encoded_attr = token_to_idx[attribute]
                    encoded_obj = id_to_idx[subj]
                    aux = []
                    aux.append(encoded_obj)
                    aux.append(encoded_attr)
                    encoded_obj_attr.append(aux)
                    attr_idx_counter += 1  # accumulate counter

        if im_to_first_attr[i] == attr_idx_counter:
            # se non ci sono oggetti con attributi
            im_to_first_attr[i] = -1
            im_to_last_attr[i] = -1
            no_attr_counter += 1
        else:
            im_to_last_attr[i] = attr_idx_counter - 1
    print('%i rel is filtered by object' % obj_filtered)
    print('%i rel is filtered by duplicate' % duplicate_filtered)

    print('%i out of %i valid images have relationships' % (len(attr_data)-no_attr_counter, len(attr_data)))
    return np.vstack(encoded_obj_attr), im_to_first_attr, im_to_last_attr
...
def sync_objects(obj_data, rel_data):
    num_img = len(obj_data)
    for i in range(num_img):
        #Per ogni immagine, mi assicuro che il record di "Obj_data" e quello di "rel_data" corrispondano alla solita immagine
        #Prendo gli oggetti presenti nell'immagine e le relazioni presenti nell'immagine
        assert(obj_data[i]['image_id'] == rel_data[i]['image_id'])
        objs = obj_data[i]['objects']
        rels = rel_data[i]['relationships']
...
def main(args):
    ...
    attr_data = json.load(open(args.attribute_input))
    assert(len(rel_data) == len(obj_data) and
           len(obj_data) == len(img_data) and
           len(attr_data) == len(img_data))
    ...
    attr_data = filter_by_idx(attr_data, valid_im_idx)
    ...
    preprocess_attribute(attr_data)
    ...
    attribute_tokens, attribute_token_counter = extract_attribute_token(attr_data)
    attribute_to_idx, idx_to_attribute = build_token_dict(attribute_tokens)


    with open("attribute_counter.json", 'w') as f:
        json.dump(attribute_token_counter, f)
    ...
    encoded_obj_attr, im_to_first_attr, im_to_last_attr = encode_attributes(attr_data, attribute_to_idx, obj_data)
    f.create_dataset('attributes', data=encoded_obj_attr)
    f.create_dataset('img_to_first_attr', data=im_to_first_attr)
    f.create_dataset('img_to_last_attr', data=im_to_last_attr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imdb', default='D:\scene-graph-TF-release-master\data_tools\imdb_1024.h5', type=str)
    parser.add_argument('--object_input', default='D:\scene-graph-TF-release-master\data_tools\VG\objects.json', type=str)
    parser.add_argument('--relationship_input', default='D:\scene-graph-TF-release-master\data_tools\VG\\relationships.json', type=str)
    parser.add_argument('--metadata_input', default='D:\scene-graph-TF-release-master\data_tools\VG\image_data.json', type=str)
    parser.add_argument('--attribute_input', default='D:\scene-graph-TF-release-master\data_tools\VG\\attributes.json', type=str)
    parser.add_argument('--object_alias', default='D:\scene-graph-TF-release-master\data_tools\VG\object_list.txt', type=str)
    parser.add_argument('--pred_alias', default='D:\scene-graph-TF-release-master\data_tools\VG\predicate_alias.txt', type=str)
    parser.add_argument('--object_list', default='D:\scene-graph-TF-release-master\data_tools\VG\object_list.txt', type=str)
    parser.add_argument('--pred_list', default='D:\scene-graph-TF-release-master\data_tools\VG\predicate_list.txt', type=str)
